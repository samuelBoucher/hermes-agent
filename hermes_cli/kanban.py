...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_gsub.add_argument("--all-boards", action="store_true",
    ...
    p_glist.add_argument("--all-boards", action="store_true",
    ...
    p_grm.add_argument("--all-boards", action="store_true",
...
def _cmd_notify_global_subscribe(args: argparse.Namespace) -> int:
    slugs = _active_board_slugs_once() if getattr(args, "all_boards", False) else [kb.get_current_board()]
    ...
    board_note = f" on {', '.join(slugs)}" if getattr(args, "all_boards", False) else " on this board"
    print(
        f"Subscribed global {args.platform}:{args.chat_id}"
        + (f":{args.thread_id}" if args.thread_id else "")
        + f" to completed/blocked Kanban events{board_note}"
    )
    return 0
